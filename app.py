...
# Limitar el número de equipos a un máximo de 20
max_equipos = 20
...
# Validar que el número de equipos no exceda el máximo
tasas_falla = list(map(float, tasas_input.strip().split(",")))
if len(tasas_falla) > max_equipos:
    st.error(f"El número máximo de equipos permitido es {max_equipos}.")
else:
